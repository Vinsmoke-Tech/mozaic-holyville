...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#item_category_id').change(function () {
            ...
            $.ajax({
                type: "GET",
                url: "{{url('option-item')}}" + '/' + this.value,
        ...
        $('#item_id').change(function () {
            ...
            $.ajax({
                type: "GET",
                url: "{{url('option-item-unit')}}" + '/' + this.value,
                success: function (data) {
        ...
        $('#item_unit_id').change(function () {
            ...
            $.ajax({
                type: "GET",
                url: "{{url('get-item-sales_price')}}" + '/' + item_id,
                success: function (data) {

                    $('#item_unit_price').val(data);

                    var quantity = $("#quantity").val();
                    var subtotal = quantity * data;
        ...
        var subtotal_amount         = $("#subtotal_amount1").val();
        var ppn_percentage_total    = $("#ppn_percentage_total").val();
        var ppn_amount_total        = ppn_percentage_total * subtotal_amount / 100;

        $("#ppn_amount_total").val(ppn_amount_total);
        $("#ppn_amount_total_view").val(toRp(ppn_amount_total));
        ...
        $('#discount_amount_view').change(function(){
            var subtotal_amount = parseFloat($("#subtotal_amount").val());
            var discount_amount_view = parseFloat($(this).val());

            // Calculate discount percentage
            var discount_percentage = (discount_amount_view / subtotal_amount) * 100;

            // Update discount percentage input field
            $('#discount_percentage').val(discount_percentage);

            // Update discount amount input field
            $('#discount_amount').val(discount_amount_view);
        ...
        $("#discount_percentage_total").change(function(){
            ...
            var ppn_amount_total            = $("#ppn_amount_total").val();
            var discount_amount_total       = (discount_percentage_total * subtotal_amount1) / 100;
            var total_amount                = parseFloat(subtotal_amount1) - parseFloat(discount_amount_total) + parseFloat(ppn_amount_total);
        ...
        <div class="row form-group">
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    {!! Form::select('payment_method',  $sales_payment_method_list, $saleinvoiceelements == null ? '' : $saleinvoiceelements['payment_method'], ['class' => 'selection-search-clear select-form', 'id' => 'payment_method', 'onchange' => 'function_elements_add(this.name , this.value);']) !!}
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <a class="text-dark">Nama Pelanggan<a class='red'> *</a></a>
                    <input class="form-control input-bb" name="customer_name" id="customer_name" type="text" autocomplete="off" onChange="function_elements_add(this.name, this.value);" value="{{$saleinvoiceelements == null ? '' : $saleinvoiceelements['customer_name']}}"/>
                        ...
                        <tbody>
                            ...
                            $ppn_amount                 = 0;
                                if (!is_array($arraydatases)) {
                                    ...
                                    $ppn_amount = $subtotal_amount * $ppn_percentage / 100;
                                }  
                            ?>
                            <tr>
                            ...
                            <tr>
                                <th colspan="3">PPN (%)</th>
                                <td style='text-align  : right !important;'>
                                    <input type="text" style="text-align  : right !important;" class="form-control input-bb" name="ppn_percentage_total" id="ppn_percentage_total" value="{{ $ppn_percentage }}" autocomplete="off" readonly/>
								</td>
                                <td style='text-align  : right !important;'>
                                    <input type="text" style="text-align  : right !important;" class="form-control input-bb" name="ppn_amount_total_view" id="ppn_amount_total_view" value="" readonly/>
                                    <input type="text" style="text-align  : right !important;" class="form-control input-bb" name="ppn_amount_total" id="ppn_amount_total" value="" hidden/>
								</td>
                                <td></td>
                                <td></td>
                            </tr>
                            <tr>
                                ...
                                <td style='text-align  : right !important;'>
                                    <input type="text" style="text-align  : right !important;" class="form-control input-bb" name="total_amount_view" id="total_amount_view" value="{{ number_format($subtotal_amount+$ppn_amount,2,'.',',') }}" readonly/>
                                    <input type="text" style="text-align  : right !important;" class="form-control input-bb" name="total_amount" id="total_amount" value="{{ $subtotal_amount+$ppn_amount }}" hidden/>
								</td>
                                <td></td>
                                <td></td>
                            </tr>
