...
use App\Models\JournalCategoriesAccount;
use DateTime;
...
class JournalSpecialController extends Controller
{
    ...
    public function index()
    {
        ...
        ->where('acct_journal_voucher.transaction_module_code', 'JK')
        ->where('acct_journal_voucher.company_id', Auth::user()->company_id)
        ->get();
        return view('content.JournalSpecial.ListJournalSpecial', compact('data','start_date','end_date'));
    }

    public function addJournalSpecial()
    {   
        ...
        $account = JournalCategories::select(DB::raw("CONCAT(journal_categories_name) AS full_account"),'journal_categories_id')
        ->where('data_state',0)
        ->where('company_id',Auth::user()->company_id)
        ->get()
        ->pluck('full_account','journal_categories_id');
        return view('content.JournalSpecial.FormAddJournalSpecial',compact('status','account','journal','arraydata'));
    }

    public function addElementsJournalSpecial(Request $request)
    {
        $journal = Session::get('journal');
        if(!$journal || $journal == ''){
    ...
    public function addArrayJournalSpecial(Request $request)
    {
        $request->validate([
            'account_id'                => 'required',
        ...
        return redirect('/journal-special/add');
    }

    public function resetAddJournalSpecial()
    {
        Session::forget('journal');
        Session::forget('arraydatases');
    ...
    public function processAddJournalSpecial(Request $request)
    {
        $transaction_module_code = 'JK';
        ...
        if(JournalVoucher::create($datases)){
            ...
            return redirect('/journal-special/add')->with('msg',$msg);
        } else {
    ...
    public function filterJournalSpecial(Request $request)
    {
        ...
        return redirect('/journal-special');
    }

    public function resetFilterJournalSpecial()
    {
    ...
    public function reverseJournalSpecial($journal_voucher_id)
    {
        ...
        try {
            ...
            return redirect()->route('journal-special');
        } catch (\Throwable $th) {
            dd($th);
            DB::rollback();
    ...
    public function printJournalSpecial($journal_voucher_id)
    {
    ...
    public function printJournalSpecialAll()
    {
        ...
        ->where('acct_journal_voucher.journal_voucher_date','>=',$start_date)
        ->where('acct_journal_voucher.journal_voucher_date','<=',$end_date)
        // ->whereIn('acct_journal_voucher_item.account_id_status',1)
    ...
    public function printJournalSpecialDebit()
    {
    ...
    public function printJournalSpecialCredit()
    {
