...
class ConsigneeReportController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = SalesConsigneeItem::where('data_state',0)
        ->where('consignee_date','>=',$start_date)
        ->where('consignee_date','<=',$end_date)
        ->where('company_id', Auth::user()->company_id)
        ->get();
            
        return view('content.ConsigneeReport.ListConsigneeReport', compact('data','start_date','end_date'));
    }

    public function filterConsigneeReport(Request $request)
    {
        ...
        return redirect('/consignee-report');
    }

    public function filterResetConsigneeReport()
    {
    ...
    public function printConsigneeReport()
    {
        if(!Session::get('start_date')){
            $start_date     = date('Y-m-d');
        }else{
        ...
        $filename = 'Laporan_Konsinyasi_'.$start_date.'s.d.'.$end_date.'.pdf';
        $pdf::Output($filename, 'I');
    }

    public function exportConsigneeReport()
    {
        if(!Session::get('start_date')){
            $start_date     = date('Y-m-d');
        }else{
        ...
        if(count($data)>=0){
            $spreadsheet->getProperties()->setCreator("IBS CJDW")
                                        ...
                                        ->setTitle("Consignee Report")
                                        ->setSubject("")
                                        ->setDescription("Consignee Report")
                                        ->setKeywords("Consignee Report")
                                        ->setCategory("Consignee Report");
            ...
            $sheet->setCellValue('B1',"Laporan Konsinyasi Dari Periode ".date('d M Y', strtotime($start_date))." s.d. ".date('d M Y', strtotime($end_date)));	
            $sheet->setCellValue('B3',"No");
            $sheet->setCellValue('C3',"Tanggal Penjualan");
            $sheet->setCellValue('D3',"Nama Barang");
            $sheet->setCellValue('E3',"Stok Konsinyasi");
            $sheet->setCellValue('F3',"Stok Laku");
            $sheet->setCellValue('G3',"Stok Tidak Laku");
            $sheet->setCellValue('H3',"Laba Kotor");
            $sheet->setCellValue('I3',"Setor");
            $sheet->setCellValue('J3',"Keuntungan"); 
            ...
            foreach($data as $key=>$val){
                ...
                if(is_numeric($key)){
                    ...
                    $sheet->setCellValue('C'.$j, $val['consignee_date']);
                    $sheet->setCellValue('D'.$j, $this->getItemName($val['item_id']));
                    $sheet->setCellValue('E'.$j, $val['quantity']);
                    $sheet->setCellValue('F'.$j, $val['item_sold']);
                    $sheet->setCellValue('G'.$j, $val['item_not_sold']);
                    $sheet->setCellValue('H'.$j, number_format($val['gross_profit'],2,'.',','));
                    $sheet->setCellValue('I'.$j, number_format($val['depositor_profit'],2,'.',','));
                    $sheet->setCellValue('J'.$j, number_format($val['consignment_profits'],2,'.',','));
                }
                $subtotal_amount += $val['consignment_profits'];
