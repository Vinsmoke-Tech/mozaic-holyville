...
class SalesConsigneeController extends Controller
{
    ...
    public function index()
    {
        ...
        ->get();
        return view('content.SalesConsignee.ListSalesConsignee',compact('data', 'start_date', 'end_date'));
    }

    public function addSalesConsignee()
    {
        ...
        $purchaseinvoiceitem = PurchaseInvoiceItem::where('data_state',0)
        ->get();

        return view('content.SalesConsignee.FormAddSalesConsignee',compact('date','categorys','items','units','arraydatases','customers', 'ppn_percentage','purchaseinvoiceitem'));
    }

    public function addArraySalesConsignee(Request $request)
    {
        $request->validate([
        ...
        return redirect('/sales-consignee/add');
    }

    public function deleteArraySalesConsignee($record_id)
    {
        $arrayBaru = array();
    ...
    // public function processAddSalesConsignee(Request $request)
    // {
    ...
    //             );  
    //             SalesConsigneeItem::create($dataarray);
    ...
    //             $debit_amount = $fields['subtotal_amount1'];
    //             $credit_amount = 0;
    //         } else {
    //             $debit_amount = 0;
    //             $credit_amount = $fields['subtotal_amount1'];
    //         }
    //         $journal_debit = array(
    //             'company_id'                    => Auth::user()->company_id,
    //             'journal_voucher_id'            => $journal_voucher_id['journal_voucher_id'],
    //             'account_id'                    => $account_id,
    //             'journal_voucher_amount'        => $fields['subtotal_amount1'],
    ...
    //         return redirect('/sales-consignee')->with('msg',$msg);
    //     } else {
    ...
    public function resetSalesConsignee()
    {
        Session::forget('arraydatases');
    ...
    public function detailSalesConsignee($sales_invoice_id)
    {
    ...
    public function deleteSalesConsignee($sales_invoice_id)
    {
        ...
        if($account_setting_status == 0){
            $debit_amount   = $sales_invoice['subtotal_amount'];
            $credit_amount  = 0;
        }else{
            $debit_amount   = 0;
            $credit_amount  = $sales_invoice['subtotal_amount'];
        }
        ...
        $journal_credit = array(
            ...
            'journal_voucher_id'            => $journal_voucher_id['journal_voucher_id'],
            'account_id'                    => $account_id,
            'journal_voucher_amount'        => $sales_invoice['subtotal_amount'],
            'account_id_default_status'     => $account_default_status,
            'account_id_status'             => $account_setting_status,
            'journal_voucher_debit_amount'  => $debit_amount,
            'journal_voucher_credit_amount' => $credit_amount,
    ...
    public function filterSalesConsignee(Request $request)
    {
        ...
        return redirect('/sales-consignee');
    }

    public function filterResetSalesConsignee()
    {
