...
class SalesInvoiceController extends Controller
{
    ...
    public function index()
    {
        ...
        Session::forget('saleinvoiceelements');
    ...
    {
        $saleinvoiceelements  = Session::get('saleinvoiceelements');
        ...
        $ppn_percentage = PreferenceCompany::select('ppn_percentage')
        ->where('company_id', Auth::user()->company_id)
        ->first()
        ->ppn_percentage;
        ...
        return view('content.SalesInvoice.FormAddSalesInvoice',compact('date','categorys','items','units','arraydatases','customers', 'ppn_percentage','sales_payment_method_list','saleinvoiceelements'));
    }

    public function elements_add(Request $request){
        $saleinvoiceelements= Session::get('saleinvoiceelements');
        if(!$saleinvoiceelements || $saleinvoiceelements == ''){
            $saleinvoiceelements['customer_name'] = '';
            $saleinvoiceelements['payment_method'] = '';
        }
        $saleinvoiceelements[$request->name] = $request->value;
        Session::put('saleinvoiceelements', $saleinvoiceelements);
    }
    ...
    public function processAddSalesInvoice(Request $request)
    {
        ...
        if (empty($request->ppn_percentage_total)){
            $ppn_percentage_total   = 0;
            $ppn_amount_total       = 0;
        }else{
            $ppn_percentage_total   = $request->ppn_percentage_total;
            $ppn_amount_total       = $request->ppn_amount_total;
        }
        $data = array(
            ...
            'ppn_percentage_total'      => $ppn_percentage_total,
            'ppn_amount_total'          => $ppn_amount_total,
        ...
        if($request->payment_method == 1){
            $journal = array(
                ...
                'journal_voucher_description'   => $this->getTransactionModuleName($transaction_module_code) ." Tunai ".$request->customer_name ." ".$sales_invoice_id['sales_invoice_no'],
                'journal_voucher_title'         => $this->getTransactionModuleName($transaction_module_code) ." Tunai ".$request->customer_name ." ".$sales_invoice_id['sales_invoice_no'],
            ...
            $journal = array(
                'company_id'                    => Auth::user()->company_id,
                'journal_voucher_status'        => 1,
                'journal_voucher_description'   => $this->getTransactionModuleName($transaction_module_code) ." Non Tunai ".$request->customer_name ." ".$sales_invoice_id['sales_invoice_no'],
                'journal_voucher_title'         => $this->getTransactionModuleName($transaction_module_code) ." Non Tunai ".$request->customer_name ." ".$sales_invoice_id['sales_invoice_no'],
                'transaction_module_id'         => $transaction_module_id,
                'transaction_module_code'       => $transaction_module_code,
                'journal_voucher_date'          => $fields['sales_invoice_date'],
                'journal_voucher_period'        => date('Ym'),
            ...
            $journal = array(
                'company_id'                    => Auth::user()->company_id,
                'journal_voucher_status'        => 1,
                'journal_voucher_description'   => $this->getTransactionModuleName($transaction_module_code) ." ".$request->customer_name ." ".$sales_invoice_id['sales_invoice_no'],
                'journal_voucher_title'         => $this->getTransactionModuleName($transaction_module_code) ." ".$request->customer_name ." ".$sales_invoice_id['sales_invoice_no'],
                'transaction_module_id'         => $transaction_module_id,
                'transaction_module_code'       => $transaction_module_code,
                'journal_voucher_date'          => $fields['sales_invoice_date'],
                'journal_voucher_period'        => date('Ym'),
        ...
        if(JournalVoucher::create($journal)){
            ...
            foreach ($arraydatases as $key => $val) {
                $dataarray = array(
                    ...
                    'ppn_percentage'                    => $ppn_percentage_total,
                    'ppn_amount'                        => $ppn_percentage_total/100*$val['item_unit_price']*$val['quantity'],
                    'discount_amount'                   => $val['discount_amount'],
                ...
                ->where('item_menu_id', $dataarray['item_id'])
                ->where('data_state', 0)
                ->get();
                // echo json_encode($receipe);exit;
                
             //Penjualan Tanpa Resep
                if(!$receipe ->count()){
                    $stock_item2 = InvtItemStock::where('item_id',$dataarray['item_id'])
                    ->where('item_unit_id', $dataarray['item_unit_id'])
                    ->where('company_id', Auth::user()->company_id)
                    ->first();

                        $stock_item2->last_balance = $stock_item2['last_balance'] - $dataarray['quantity'];
                        $stock_item2->updated_id = Auth::id();
                        $stock_item2->save();

            //Penjualan Menggunakan Resep
                    }else{
                    foreach($receipe as $val){
                        ...
                        ->first();
                            ...
                            if(isset($stock_item)){
                                $stock_item->last_balance   = $stock_item['last_balance'] - $val['quantity'] * $dataarray['quantity'];
            ...
            // $journal_voucher_id = JournalVoucher::orderBy('created_at', 'DESC')->where('company_id', Auth::user()->company_id)->first();
            // if ($account_setting_status == 0){
            //     $debit_amount = $ppn_amount_total;
            //     $credit_amount = 0;
            // } else {
            //     $debit_amount = 0;
            //     $credit_amount = $ppn_amount_total;
            // }
            // $journal_credit = array(
            //     'company_id'                    => Auth::user()->company_id,
            //     'journal_voucher_id'            => $journal_voucher_id['journal_voucher_id'],
            //     'account_id'                    => $account_id,
            //     'journal_voucher_amount'        => $ppn_amount_total,
            //     'account_id_default_status'     => $account_default_status,
            //     'account_id_status'             => $account_setting_status,
            //     'journal_voucher_debit_amount'  => $debit_amount,
            //     'journal_voucher_credit_amount' => $credit_amount,
    ...
    {
        $transaction_module_code    = 'HPJL';
        $transaction_module_id      = $this->getTransactionModuleID($transaction_module_code);
        $sales_invoice              = SalesInvoice::where('sales_invoice_id', $sales_invoice_id)->first();
        $journal_voucher            = JournalVoucherItem::where('created_at', $sales_invoice['created_at'])
        ->where('company_id',Auth::user()->company_id)
        ->first();
        // dd($journal_voucher);exit;
        ...
        // if ($account_setting_status == 0){
        //     $debit_amount =  $sales_invoice['ppn_amount_total'];
        //     $credit_amount = 0;
        // } else {
        //     $debit_amount = 0;
        //     $credit_amount = $sales_invoice['ppn_amount_total'];
        // }
        // $journal_credit = array(
        //     'company_id'                    => Auth::user()->company_id,
        //     'journal_voucher_id'            => $journal_voucher_id['journal_voucher_id'],
        //     'account_id'                    => $account_id,
        //     'journal_voucher_amount'        => $sales_invoice['ppn_amount_total'],
        //     'account_id_default_status'     => $account_default_status,
        //     'account_id_status'             => $account_setting_status,
        //     'journal_voucher_debit_amount'  => $debit_amount,
        //     'journal_voucher_credit_amount' => $credit_amount,
