...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // echo json_encode($item_data);exit;
    ...
    public function selectItemStock($category_id, $unit_id, $item_id)
    {
    ...
    public function selectItemCost($item_id)
    {
        $unit = InvtItem::select('item_unit_cost')
    ...
    public function selectItemPrice($item_id)
    {
        $data = InvtItem::select('item_unit_price')
        ->where('item_unit_id','!=',null)
        ->where('item_id', $item_id)
        ->where('data_state',0)
        ->where('company_id', Auth::user()->company_id)
    ...
    public function getMarginCategory($item_id)
    {
        $category_id = InvtItem::select('item_category_id')
        ->where('item_unit_id','!=',null)
        ->where('item_id', $item_id)
        ->where('data_state',0)
        ->where('company_id', Auth::user()->company_id)
    ...
    {
        $data_package = InvtItem::where('item_id', $item_id)
        ->where('company_id', Auth::user()->company_id)
        ->where('item_unit_id','!=', null)
        ->get();
